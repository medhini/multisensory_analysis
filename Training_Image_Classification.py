...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        transform_image = np.zeros((self.frames_len,3,224,224))
        if self.transform:
            for i in range(self.frames_len):
                transform_image[i] = self.transform(image[i]) # Transform image frames
        ...
        return (transform_image,audio,onehot_label)
...
class alignment(nn.Module):
    def __init__(self):
        ...
        self.conv3_2 = nn.Conv3d(64, 512, (1, 1, 1))
    ...
    def forward(self, sounds, images):
        ...
        images = images.view(batchsize, 3, num, xd, yd)
...
try:
    ...
    print("interrupted")
    torch.save(model_align, 'ucf_image_classification_200.pth')
torch.save(model_align, 'ucf_image_classification_200.pth')
